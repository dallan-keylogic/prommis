...
reo_list = ["Sc2O3", "Y2O3", "La2O3", "Ce2O3", "Pr2O3", "Nd2O3", "Sm2O3", "Gd2O3", "Dy2O3"]
...
class CoalRefusePropertiesScaler(CustomScalerBase):
    ...
    def constraint_scaling_routine(
        ...
        for idx, con in model.conversion_eq.items():
            sf = get_scaling_factor(model.mass_frac_comp[idx])
            self.set_constraint_scaling_factor(con, sf, overwrite=overwrite)
    
        if model.is_property_constructed("sum_mass_frac"):
            self.set_constraint_scaling_factor(model.sum_mass_frac, 1, overwrite=overwrite)
